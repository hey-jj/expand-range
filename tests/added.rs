...
use expand_range::{fill, fill_checked, FillError, FillResult, Options, Step, Value};
...
fn single_invalid_start_is_empty() {
    exact(fill(Value::from(""), None, Step::None, Options::new()), &[]);
}

#[test]
fn single_valid_value_returns_itself() {
    exact(
        fill(Value::from(5), None, Step::None, Options::new()),
        &[n(5.0)],
    );
    exact(
        fill(Value::from("1"), None, Step::None, Options::new()),
        &[s("1")],
    );
    exact(
        fill(Value::from("a"), None, Step::None, Options::new()),
        &[s("a")],
    );
    exact(
        fill(Value::from(0), None, Step::None, Options::new()),
        &[n(0.0)],
    );
}
...
fn letter_regex_equal_bounds_is_single_char() {
    ...
    o.to_regex = true;
    regex_eq(
        fill(Value::from("a"), Some(Value::from("a")), Step::None, o),
        "a",
    );
}
...
fn letter_regex_with_step_ignores_wrap() {
    ...
    o.capture = true;
    regex_eq(
        fill(Value::from("a"), Some(Value::from("z")), Step::from(3), o),
        "a|d|g|j|m|p|s|v|y",
    );
}
...
fn wrap_without_to_regex_is_noop() {
    ...
    o.wrap = true;
    exact(
        fill(Value::from(1), Some(Value::from(3)), Step::None, o),
        &[n(1.0), n(2.0), n(3.0)],
    );
}
...
fn negative_zero_does_not_leak_into_regex() {
    let mut o = Options::new();
    o.to_regex = true;
    let r = fill(Value::from(-5), Some(Value::from(5)), Step::None, o);
